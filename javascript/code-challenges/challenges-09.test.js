...
const updateNumbers = (obj) => {
  // Solution code here...
  let properties3 = Object.keys(obj);
  let properties2 = Object.values(obj);
  let arr = [];
  properties3.forEach((item, idx) => {
    arr.push(item + ': ' + properties2[idx]);
  });
  return arr;
};
​
...
const getHouses = (arr) => {
  let x=[];
  arr.map(value=>x.push(value.house));
  return x;
};
